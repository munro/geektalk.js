...
screen.scroll = function (amount) {
    current_message += amount;
    current_message = Math.min(current_message, messages.length - 1);
    current_message = Math.max(current_message, 0);
    screen.redrawScreen();
};
...
screen.systemMessage = function (type, message, timestamp) {
    ...
    screen.message('|' + timestamp + type + '|' + message); 
};
